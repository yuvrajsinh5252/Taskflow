...
function GanttChart() {
  const { GanttInfo } = React.useContext(
    GanttChartContext
  ) as GanntChartContextType;
  const { status, setStatus, speed } = React.useContext(
    ControllerContext
    ...
    const interval = setInterval(() => {
      ...
      if (i < GanttInfo.length - 1) {
        if (status == "running") {
          ...
          setWidth(
            `${
              (GanttInfo[i].Interval[1] - GanttInfo[i].Interval[0] + 1) * 20
            }px`
          );
          i++;
        }
        ...
        } else if (visibleProcesses.length > 1 && i == 0)
          setVisibleProcesses([]);
        else if (status == "next") {
          setVisibleProcesses((prev) => [...prev, GanttInfo[i]]);
          setWidth(
        ...
        setVisibleProcesses((prev) => prev.slice(0, -1));
        setWidth(
          `${(GanttInfo[i].Interval[1] - GanttInfo[i].Interval[0] + 1) * 20}px`
        );
        i--;
        setStatus("reset");
        localStorage.setItem("i", i);
